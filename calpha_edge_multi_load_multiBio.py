...
def computeCalphaDistanceResidue(residue1, residue2, residue_to_atoms):
    calpha_residue1 = [atom for atom in residue_to_atoms[residue1] if atom.fullname == "CA"][0]
    calpha_residue2 = [atom for atom in residue_to_atoms[residue2] if atom.fullname == "CA"][0]
...
def calphaLoadGraphFromFile(filename, name_to_pattern, classification_type="helices", distance_based=True, distance_threshold=2, nb_neighbors=2, nb_features=4, shuffle=False, nmr_conformations=False):
    ...
    for i in range(len(residues)-1):
        ...
        for j in range(len(residues)-1):
            if i != j:
                ...
                dij = computeCalphaDistanceResidue(residue_i, residue_j, residue_to_atoms)
    ...
    #threshold = 10 # Angström
    #indices_closest_neighbors = indices_closest_neighbors.tolist()
    #indices_closest_neighbors = [[x for x in icn if DISTANCES[i_icn][x] <= threshold] for i_icn, icn in enumerate(indices_closest_neighbors)]
    ...
    for i in range(len(residues)-1):
        #nb_neighbors = len(indices_closest_neighbors[i])
        FOS_i = []
        try:
        ...
        for inj, nj in enumerate(indices_closest_neighbors[i][:nb_neighbors]): 
            ...
            phi_ij = clifford_algebra.compute_formula(x_, y_, a_, b_, c_, d_) 

            edge_ij_features = [phi_ij]
            A_row1.append(i)
            A_row2.append(nj)
        ...
        mean_phi_i = np.mean([x[0] for x in FOS_i])
        std_phi_i = np.std([x[0] for x in FOS_i])
        features_FOS_i += [mean_phi_i, std_phi_i]
...
def calphaLoadGraphsFromDirectory(filedir, name_to_pattern, classification_type="helices", distance_based=True, distance_threshold=5, nb_neighbors=2, nb_features=4, shuffle=False, nmr_conformations=False):
            ...
            for _, _, files in os.walk(subdir):
                for i_protein, protein_filename in enumerate(files):
                        if '.cif' in protein_filename:
                            ...
                            sys.stdout.flush()
                            A, X, Y, NX, FOS = calphaLoadGraphFromFile(subdir + "/"  + protein_filename, name_to_pattern, classification_type=classification_type, distance_based=distance_based, distance_threshold=distance_threshold, nb_neighbors=nb_neighbors, nb_features=nb_features, shuffle=shuffle, nmr_conformations=nmr_conformations)
                            if len(A) > 0:
                                As.append(A)
                                Xs.append(X)
                                Ys.append(Y)
...
if __name__ == "__main__":
    ...
    filename = sys.argv[1]
    A, X, Y, NX, features_FOS = calphaLoadGraphFromFile(filename, name_to_pattern)
    pdb.set_trace()
